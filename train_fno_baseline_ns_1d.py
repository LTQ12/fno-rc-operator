...
def main(args):
    ...
    # For this dataset, we use the same data for training and testing but shuffle it.
    # In a real scenario, one would use different simulations for testing.
    ...
    model = FNO1d(args.modes, args.width, in_channels=args.T_in, out_channels=args.T_out).to(device)
    print(f"Model: Baseline FNO1d")
    ...
    for ep in range(args.epochs):
        ...
        for x, y in train_loader:
            ...
            # The normalizer and loss function expect (batch, S, T)
    ...
    model_save_path = os.path.join(args.model_save_dir, 'fno_baseline_1d.pt')
    torch.save(model.state_dict(), model_save_path)
    print(f"--- Training Finished ---\nModel saved to {model_save_path}")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train 1D Baseline FNO')
