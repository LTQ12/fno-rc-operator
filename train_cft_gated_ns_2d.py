"""
Trainer for the CFT-Gated Fourier Neural Operator for 2D Navier-Stokes.
This is the new proposed architecture based on gating principles.
"""
import torch
import numpy as np
...
# Import the new model definition and utilities
from fourier_2d_cft_gated import FNO_CFT_Gated
...
def main(args):
    ...
    print("Loading data...")
    try:
        # Assuming data is on CPU and needs to be moved to device later
    ...
    for ep in range(args.epochs):
        ...
        for x, y in train_loader:
            ...
            # Loss is computed on normalized data for stability
        ...
        with torch.no_grad():
            for x, y in test_loader:
                x, y = x.to(device), y.to(device)
                out = model(x)
                # Decode for final error calculation
                out_decoded = y_normalizer.decode(out.cpu()).to(device)
                y_decoded = y_normalizer.decode(y.cpu()).to(device)
                test_l2 += loss_func(out_decoded.view(out.size(0), -1), y_decoded.view(y.size(0), -1)).item()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train CFT-Gated FNO.')
    
    parser.add_argument('--data_path', type=str, default='/content/drive/MyDrive/ns_data_N600_clean.pt', help='Path to data file.')
    parser.add_argument('--model_save_path', type=str, default='models/cft_gated_fno.pt', help='Path to save the trained model.')
    ...
    # Hyperparameters aligned with baseline FNO for a fair comparison
