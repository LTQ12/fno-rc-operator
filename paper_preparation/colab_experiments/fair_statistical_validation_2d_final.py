"""
公平的FNO-RC 2D Navier-Stokes统计显著性验证实验 - 最终修复版本
严格确保两个模型使用完全相同的参数和训练设置
使用PyTorch标准Adam优化器以避免复数权重问题
"""
...
def train_model_fair(model, data, device, epochs=100, ntrain=500, ntest=100, T_in=10, T_out=10, 
    ...
    print(f"Optimizer: {type(optimizer).__name__} (PyTorch标准版本，与原始实验参数一致)")
...
def run_fair_statistical_experiments():
    ...
    results = {
        ...
        'metadata': {
            ...
            'note': 'Fair comparison with identical hyperparameters, using PyTorch Adam for complex weight compatibility'
        }
    }
    
    # 保存结果
    results_path = f"{base_path}/results/statistical_validation_2d/fair_2d_statistical_results_final.json"
...
def create_fair_comparison_plot(fno_errors, fno_rc_errors, results, base_path):
    ...
    plt.tight_layout()
    plt.savefig(f"{base_path}/results/statistical_validation_2d/fair_statistical_comparison_final.png", 
