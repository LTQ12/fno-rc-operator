"""
公平的FNO-RC 2D Navier-Stokes统计显著性验证实验 - 修复版本
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if grad.is_complex():
                    ...
                    # 处理实部
                    if grad_real.dtype in {torch.float16, torch.bfloat16}:
                        grad_real = grad_real.float()
                    # 处理虚部  
                    if grad_imag.dtype in {torch.float16, torch.bfloat16}:
                        grad_imag = grad_imag.float()
                        
                    grad = torch.complex(grad_real, grad_imag)
                else:
                    # 处理实数梯度
                ...
                # State initialization - 支持复数参数
                if len(state) == 0:
                    state['step'] = 0
                    # Exponential moving average of gradient values
                    if p.data.is_complex():
                        state['exp_avg'] = torch.zeros_like(p.data)
                        state['exp_avg_sq'] = torch.zeros_like(p.data.real).float()  # 平方后变为实数
                    else:
                    ...
                    if amsgrad:
                        ...
                        if p.data.is_complex():
                            state['max_exp_avg_sq'] = torch.zeros_like(p.data.real).float()
                        else:
                ...
                if p.data.is_complex():
                    ...
                    # 更新二阶矩估计（使用梯度的模长平方）
                    grad_abs_sq = (grad * grad.conj()).real  # |grad|^2
                    exp_avg_sq.mul_(beta2).add_(grad_abs_sq, alpha=1 - beta2)
                    ...
                    # 复数参数更新：分别处理实部和虚部
                    # 扩展denom以匹配复数张量的形状
                    if len(denom.shape) < len(exp_avg.shape):
                        for _ in range(len(exp_avg.shape) - len(denom.shape)):
                            denom = denom.unsqueeze(-1)
                    denom = denom.expand_as(exp_avg.real)
                    
                    # 分别更新实部和虚部
                    real_update = exp_avg.real / denom
                    imag_update = exp_avg.imag / denom
                    
                    p.data.real.add_(real_update, alpha=-step_size)
                    p.data.imag.add_(imag_update, alpha=-step_size)
...
def train_model_fair(model, data, device, epochs=100, ntrain=500, ntest=100, T_in=10, T_out=10, 
    ...
    print(f"Optimizer: {type(optimizer).__name__} (确保与标准FNO完全一致)")
...
if __name__ == "__main__":
    print("🚀 公平的FNO-RC 2D Navier-Stokes统计验证 - 修复版本")
