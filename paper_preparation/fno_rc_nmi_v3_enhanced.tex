...
\begin{tikzpicture}[
    ...
    box/.style={rectangle, draw=black, line width=0.6pt, minimum width=2.4cm, minimum height=0.6cm, align=center, font=\scriptsize},
    fno/.style={fill=green!12},
    cft/.style={fill=orange!12},
    io/.style={fill=gray!8},
    merge/.style={fill=blue!8},
    arr/.style={-stealth, line width=0.6pt, black}
]

% Input
\node[box, io, minimum width=6cm] (in) {Input $u^{(l)}(x,t)$};

% FNO path
\node[box, fno, below left=0.4cm and 1.8cm of in] (f1) {FFT};
\node[box, fno, below=of f1] (f2) {Spectral Conv\\$R_\phi \cdot \hat{u}$};
\node[box, fno, below=of f2] (f3) {IFFT};
\node[box, fno, below=of f3] (f4) {Local $W$};

% CFT path
\node[box, cft, below right=0.4cm and 1.8cm of in] (c1) {Segment\\$L$ parts};
\node[box, cft, below=of c1] (c2) {Chebyshev\\$T_m(s)$};
\node[box, cft, below=of c2] (c3) {CFT $c_{\ell m}$};
\node[box, cft, below=of c3] (c4) {MLP $g_\theta$};

% Merge
\node[box, merge, below=0.5cm of in, yshift=-4.5cm, minimum width=6cm] (add) {$v_{\text{FNO}} + \gamma \mathcal{R}_{\text{CFT}}$};
\node[box, io, below=of add, minimum width=6cm] (out) {Output $u^{(l+1)}$};
...
% Labels
\node[left=0.05cm of f2, green!60!black, font=\tiny\bfseries, rotate=90] {FNO Branch};
\node[right=0.05cm of c2, orange!70!black, font=\tiny\bfseries, rotate=90] {CFT Branch};
\node[right=0.1cm of add, font=\tiny] {learnable $\gamma$};

\end{tikzpicture}
\caption{\textbf{FNO-RC dual-path architecture.} Input splits into parallel branches. \textbf{FNO path (left, green):} Fast Fourier Transform → spectral convolution with learnable weights $R_\phi$ → inverse FFT → point-wise convolution $W$. \textbf{CFT path (right, orange):} Domain segmentation into $L$ intervals → Chebyshev polynomial expansion $T_m(s)$ → continuous Fourier coefficients $c_{\ell m}$ → MLP projection $g_\theta$. \textbf{Fusion (blue):} Outputs merge via learnable residual scaling $\gamma$, then passed to next layer. CFT correction activates in shallow layers only (first 1-2 layers) to capture high-frequency features missed by DFT.}
