"""
Trainer for 2D Fourier Neural Operator for the Navier-Stokes equation

This script is designed to train and evaluate the standard 2D FNO model
for the Navier-Stokes equation, time-dependent problem. It serves as the 
baseline for comparing against more advanced FNO architectures.
"""
import torch
import numpy as np
...
def main(args):
    ...
    if torch.isnan(data).any() or torch.isinf(data).any():
        print("!!! WARNING: NaNs or Infs found in raw data file !!!")
        return
    else:
        print("Raw data integrity check passed. No NaNs or Infs.")
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(data)):
        ...
        model = FNO2d(args.modes, args.modes, args.width, in_channels=args.T_in, out_channels=args.T_out).to(device)
        ...
        for ep in range(args.epochs):
            ...
            t2 = default_timer()
            if (ep + 1) % 50 == 0: # Print less frequently for K-fold
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train a 2D FNO for Navier-Stokes with K-Fold Cross-Validation.')
    
    # Paths and data options
    parser.add_argument('--data_path', type=str, default='data/ns_data.mat', help='Path to the .mat or .pt data file.')
    parser.add_argument('--model_path', type=str, default='models/fno_ns_2d_N600.pt', help='Path to save the trained model.')
